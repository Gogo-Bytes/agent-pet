...
状态：**P1 核心交互已获用户实机确认；顶部菜单栏入口及发布/扩展矩阵仍待验收，未打包发布。** 自动化与隔离测试记录保留如下，历史工具阻塞不代表用户后续验收未发生。本阶段仅 P1，不代表 P2 一键接入已完成。
...
用户已确认新版本能启动、关闭后可点击 Dock 恢复。顶部托盘保持独立待验收项，不能因 Dock 已恢复就关闭。

继续在临时 `/tmp/ap-tray-check.ZZHj02`、独立 profile、无 pi endpoint/token 下做原生对照（Electron 36.9.5）：

- 实际产品等待 3 秒后关闭管理窗口，托盘未销毁但 bounds 仍在屏幕下边界之外，Dock=true。[基线](evidence/desktop-p1/tray-product-baseline.json)
- 仅替换为 macOS 的 NSActionTemplate 系统图标，无改善。[系统图标](evidence/desktop-p1/tray-system-image.json)
- 仅延后 1 秒用原图像销毁/重建托盘，无改善。[延后创建](evidence/desktop-p1/tray-delayed.json)
- 完全不加载 Pet 代码、创建独立 Tray + 系统图标 + AP Test 标题，仍报告 (0,1169,109,22)，屏幕 (0,0,1800,1169)。[最小直接启动](evidence/desktop-p1/tray-minimal-direct.json)
- 同一最小程序通过 macOS Launch Services (`open -W -n -a`) 启动，仍相同。[系统启动](evidence/desktop-p1/tray-minimal-launchservices.json)

初步排除了仅由 Pet 图像或初始化时序解释的假设，**未证明具体系统设置或 Electron 缺陷是根因**。此处用 Tray.getBounds/屏幕交集作为诊断信号，不等于像素级可见性与真实鼠标点击；用户此前菜单栏不可见反馈为独立证据。

没有改生产实现，没有为对照修改登录项、用户菜单栏设置或实际偏好；所有本次测试进程已退出。需要用户确认是否有隐藏菜单栏项目的工具/设置，再收集相应可见性证据。不能仅凭这些对照盲目升级 Electron、重写图标或改变用户系统配置。

## 用户最终确认的核心闭环

以下依据本次用户对话逐项确认，不将用户确认扩大为工具或其它场景的验收：

| 项目 | 结果 |
| --- | --- |
| 启动、关闭管理窗口后通过底部 Dock 恢复 | 用户确认通过 |
| 管理窗口控制宠物显示/隐藏 | 用户确认正常 |
| 宠物工具栏调整尺寸同步到管理窗口 | 用户确认通过 |
| 管理窗口调整尺寸同步到宠物 | 初次失败；`e81f82c` 修复禁用/异步保存后，用户确认“可以了” |
| 退出重开保留尺寸与显隐状态 | 用户确认通过 |
| 退出后可再次正常启动 | `b9fcfb9` 修复退出残留后，用户确认通过 |

尺寸修复及最终 27 文件/162 项、typecheck/build 验证见 [尺寸控件报告](DESKTOP-P1-SIZE-CONTROL.md)。未将其写成顶部托盘、所有退出时机/Adapter listener、键盘/全屏/多屏或打包登录项也通过。

P1 可以作为核心基础进入下一阶段；顶部菜单栏入口为已知待处理项，Dock 是已确认恢复入口。P2 不依赖宣称所有原生矩阵已通过。
