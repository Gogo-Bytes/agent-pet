...
Status: implementation, independent code/retrievability review and parent-run verification complete for this bounded slice; no blocking slice findings. Production gates below remain open.
...
| `native/managed-darwin/{native-check,lease-child}.mjs`, `fixture-acl.c` | Isolated native tests and separately compiled test-only ACL fixture writer; the fixture writer is not a product helper |
...
Actual tests cover same-process duplicate opens, independently opened process descriptors, a live nonresponding holder, explicit close, GC, child-process exit and SIGKILL. Child exit does not independently prove Worker/environment cleanup while the host process stays alive; that remains unverified. Closing the parent root alone does not release the live lease. Process death releases the lock, while a synthetic `owner/` remains untouched. **This does not implement recovery or prove writer exclusivity for P2b.1, which is not integrated.** A kernel lease must complement, never replace, the durable uncertainty barrier.
