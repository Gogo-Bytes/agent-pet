...
export function App({ store }: { store: PetStore }) {
  ...
  useEffect(() => { void store.bridge.bubblesVisible(!hidden); }, [store, hidden]);
  ...
  return <>
    ...
    <Toolbar bridge={store.bridge} rect={layout?.toolbar} hidden={hidden} onToggle={() => setHidden(value => !value)} />
    <p className="pet-notice" role="status" style={noticeStyle}>{message ?? (error || modelNotice)}</p>
  </>;
}
