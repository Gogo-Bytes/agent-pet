...
状态：pi 的本机静态接口核实完成，I1a、I1b 和 I1c 的扩展实现已完成；现在只剩用户授权加载后的真实 TUI 验收。Claude Code / Codex 官方全文与版本仍待核实。实施顺序：pi → Claude Code → Codex，后两项不越过各自证据门槛。尚未宣称任何真实 Agent 运行时验收通过。
...
- **I1c：用户授权后的真实验证（扩展已实现，验收待用户执行）**。桌面已能通过显式 endpoint/token 启动 Adapter；只读扩展位于 `integrations/pi-extension/index.ts`，需由用户主动加载到闲置的已有 TUI，再实测多轮工作、取消、重试和重连；未验收前不进入 Claude Code。
